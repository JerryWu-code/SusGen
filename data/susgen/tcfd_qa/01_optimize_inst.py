...
def replace_prefix(item):
    ...
    if split_point in instruction:
        ...
        item['input'] = questions if random.random() < 0.5 else split_point + questions
        return item
    else:
        return item  # Return as is if the split point isn't found
...
with open('data/susgen/tcfd_qa/tcfd_qa_v4.json', 'w') as file:
